...
> **難読化であり、暗号化ではありません。** Kasumi は ID の構造を隠しますが、暗号としての安全性は持ちません。salt の空間は 32 ビット（約 20 億通り）であり、既知の平文ペアが十分に得られればブルートフォース可能です。Kasumi だけを不正アクセス対策の唯一の防壁にしないでください。サーバーサイドの認可チェックは必ず別途実装してください。
